...
#check if rounds are correct 
table(csew_nvf$round) #good 

rm("csew_nvf_18", "csew_nvf_19", "csew_nvf_20")

#Police attitudes variables 
#Reverse code 
...
#CFA operationalising trust in the police construct 
#One factor solution 
...
#ask for the parameter estimates to be placed on the paths directly
...
#Create a latent variable in NVF dataset for 1 factor model 
csew_nvf$trust <- as.data.frame(lavPredict(trust.fit1))$trust

#Create two latent variables in NVF dataset for 2 factor model 
...
#recode CFA scores to positive 0-1 values
...
#Correlation test
cor.test(~ effectiveness + procedural_justice, data = csew_nvf) #0.93
...
#ow no=0 - not a repeat violnce victim in last 12 months
...
#Grouped by reported, without missing values -completed cases only  
...
#Nagelkerke's will typically be higher than McFadden
NagelkerkeR2(m1) #0.49%
NagelkerkeR2(m2) #2.31%
NagelkerkeR2(m3) #4.12%
NagelkerkeR2(m4) #5.57%

#Tjur's R2 automatically calculated in table output 
...
############################################################################
#one factor trust logistic regression 
...
#All incident based analysis should be weighted by 'c11weighti'
#weight design 
data_w <- csew_full %>% 
  as_survey(weights = c(C11weighti))

summary(csew_full$C11weighti)

m1_w <- svyglm(reported ~ effectiveness_recoded + 
            repeat_victim + series + survey_round, 
            design = data_w, data = csew_full, family=quasibinomial)

summary(m1_w)
vif(m1_w)

#With victim characteristics 
m2_w <- svyglm(reported ~ effectiveness_recoded + 
            ethnicity + sex + age + education +
            repeat_victim + series + survey_round, 
            design = data_w, data = csew_full, family=quasibinomial)
...
#With situational characteristics 
m3_w <- svyglm(reported ~ effectiveness_recoded +
            attempted + injury + weapon + multiple_offenders +
            stranger +
            repeat_victim  + series + survey_round,
          design = data_w, data = csew_full, family=quasibinomial)
...
m4_w <- svyglm(reported ~ effectiveness_recoded +
                 ethnicity + sex + age + education +
                 attempted + injury + weapon + multiple_offenders + 
                 stranger + 
                 repeat_victim  + series + survey_round, 
...
m1_w <- m1_w %>% 
  ...
  add_significance_stars(hide_ci = FALSE, hide_se = TRUE, hide_p = TRUE)
